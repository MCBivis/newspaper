        ...
        <Row gutter={16}>
          {tableProps.dataSource?.map((record: BaseRecord) => (
              <Col span={8} key={record.id}>
                <Card
                    title={`Выпуск ${new Date(record.PublishDate).toLocaleDateString("ru-RU")}`} // Use the date for the title
                ...
                >
                  ...
                  <p><strong>Газета:</strong> {record.newspaper?.name}</p>
                  <Tag color={record.status === "in_progress" ? "orange" : "green"}>
                    {record.status}
                  </Tag>
                </Card>
              </Col>
