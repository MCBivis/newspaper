...
export const Grid: FC<GridProps> = ({
  ...
  const issueDateBeautified = new Intl.DateTimeFormat("ru-RU", {
    year: "numeric",
    month: "long",
    day: "numeric",
  ...
  const openSplitTextModal = (widgetId: string) => {
    ...
    const originalTitle = widget.content?.title || widget.content?.name || "Без названия";
  ...
  const handleSplitText = () => {
    ...
    if (splitIndices.length === 0) {
      message.error("Выберите хотя бы одну точку разделения");
      return;
    }
    ...
    const newItems = parts.map((part, index) => ({
      ...
      title: `Часть ${index + 1} из ${parts.length} — ${originalTitle || "Без названия"}`,
      content: part,
      originalArticleName: originalTitle || "Без названия",
    }));
    ...
    message.success(`Текст разделен на ${parts.length} частей`);
    ...
    try {
      ...
      const imagesArray: { name: string; url: string; id: number }[] =
        photosData.data?.data.data
          ...
          .map((photo: any) => ({
            name: photo.attributes.name || "Изображение без названия",
  ...
  const renderContentList = () => {
    ...
    return (
      <List
        ...
        renderItem={(item) => {
          const actions = [
            <Tooltip key="add-text" title="Добавить в макет">
              <Button
                type="primary"
                shape="circle"
                icon={<PlusOutlined />}
      ...
      renderItem={(item) => (
        <List.Item>
          <Card
            ...
            actions={[
              <Tooltip key="add-image" title="Добавить в макет">
                <Button
                  type="primary"
                  shape="circle"
                  icon={<PlusOutlined />}
  ...
  const renderAdvertisementList = () => (
    <List
      ...
      renderItem={(item) => (
        <List.Item
          actions={[
            <Tooltip key="add-ad" title="Добавить в макет">
              <Button
                type="primary"
                shape="circle"
                icon={<PlusOutlined />}
  ...
  const handleRemoveWidget = (id: string) => {
    ...
    if (widget) {
      ...
      if (isTextWidget) {
        ...
        if (!originalArticleName && widget.content?.title && (widget.content.title.startsWith('Часть') || widget.content.title.startsWith('Part'))) {
          const match = widget.content.title.match(/— (.+)$/);
          if (match) originalArticleName = match[1].trim();
        }
        if (textContent && textContent.trim()) {
          let title = "Удаленный текст";
          ...
          if (isTextPart) {
            ...
            title = `Часть ${nextPartNumber} (возвращено)`;
          }
          setTemporaryParts((prev) => {
            const newItems = [
              ...prev,
  ...
  const handleRestoreArticle = async (originalArticleName: string) => {
    ...
    if (articleParts.length === 0) {
        message.error("Части для этой статьи не найдены");
      return;
    }
    ...
    try {
      ...
      if (!data.data || data.data.length === 0) {
        message.error(`Статья "${originalArticleName}" не найдена на сервере`);
        return;
      }
      ...
      message.success(`Статья "${originalArticleName}" восстановлена`);
    } catch (error) {
      console.error("Error restoring article:", error);
      message.error("Ошибка восстановления статьи с сервера");
    }
  };
  ...
  const handleGeneratePDF = async () => {
    ...
    if (issueId) {
      ...
      message.open({
        key,
        type: "loading",
        content: "Подготовка к генерации PDF...",
      });
      ...
      try {
          ...
          (progress) => {
            if (typeof progress === "number") {
              message.open({
                key,
                type: "loading",
                content: `Генерация страницы ${progress} из ${totalPages}...`,
              });
            } else if (typeof progress === "string") {
              message.open({
                key,
                type: "success",
                content: `PDF успешно создан и сохранен как ${progress}`,
                duration: 5,
              });
            }
          }
        ...
        message.open({
          key,
          type: "error",
          content: "Произошла ошибка при генерации PDF.",
          duration: 5,
        });
      }
    }
  ...
  const renderTemporaryContentList = () => {
    ...
    const groupedParts = temporaryParts.reduce((groups, item) => {
      const articleName = item.originalArticleName || "Неизвестная статья";
          ...
          <div key={articleName} style={{ marginBottom: 16 }}>
            <div style={{ 
              ...
              <span style={{ fontWeight: 600, fontSize: '14px' }}>
                {articleName} ({parts.length} частей)
              </span>
              <Button
                type="primary"
                size="small"
            ...
            <List
              ...
              renderItem={(item) => {
                const actions = [
                  ...
                  <Tooltip key="delete-text" title="Удалить навсегда">
                    <Button
                      danger
                      shape="circle"
                      icon={<DeleteOutlined />}
    ...
    Object.entries(allLayouts).forEach(([pageKey, pageLayout]) => {
      ...
      if (textWidgets.length > 0) {
        ...
        textWidgets.forEach((widget, index) => {
          ...
          if (title && title !== "Удаленный текст") {
    ...
    if (tableOfContents.length === 1) {
          ...
          blocks: [
            {
              id: "toc-empty",
              data: {
                text: "### 📭 Статьи не найдены\n\n*Добавьте статьи на другие страницы, чтобы увидеть их здесь.*",
              },
              type: "paragraph",
            },
          ],
      ...
      const totalArticles = Object.entries(allLayouts).reduce((total, [pageKey, pageLayout]) => {
        ...
        const textWidgets = pageLayout.filter(widget => 
          ...
          widget.content?.title && 
          !widget.lock &&
          widget.content.title !== "Удаленный текст"
        );
  ...
  const handleGenerateTOC = () => {
    if (currentPageNumber === 2) {
      ...
      if (newTableOfContents && newTableOfContents.length > 0) {
        onChangeLayout(newTableOfContents);
        message.success("Оглавление успешно создано!");
      } else {
        message.warning("Статьи не найдены для создания оглавления.");
      }
    } else {
      message.warning("Оглавление можно создать только на странице 2.");
    }
  };
  ...
  return (
    <Container>
      <Sidebar>
        <h3 style={{ marginBottom: 16 }}>Список содержимого</h3>
        {currentPageNumber === 1 && (
          <div
            style={{
          ...
          >
            ...
            <br />
            Эта страница недоступна для редактирования. Она содержит обложку выпуска.
          </div>
        )}
        {currentPageNumber === 2 && (
          <div
          ...
          >
            ...
            <br />
            Эта страница недоступна для редактирования. Она содержит оглавление выпуска.
          </div>
        )}
        {(currentPageNumber !== 1 && currentPageNumber !== 2) && (
          <>
            <Divider orientation="left">Доступное содержимое</Divider>
            {renderContentList()}
            <Divider orientation="left">Временное содержимое</Divider>
            {renderTemporaryContentList()}
            <Divider orientation="left">Изображения</Divider>
            {renderImageList()}
            <Divider orientation="left">Реклама</Divider>
                          ...
                          <>
                            ...
                            {child.content?.type !== "image" && (
                              <button
                                ...
                                title="Разделить текст на части"
                              >
                                ✂
                              </button>
                            )}
      ...
      >
        <div style={{ marginBottom: 16 }}>
          <p><strong>Нажмите между словами, чтобы выбрать место разделения:</strong></p>
          <div
            style={{
              maxHeight: 200,
              overflow: "auto",
                  ...
                  if (i < words.length - 1) {
                    ...
                    return [
                      ...
                      <span
                        ...
                        title={isActive ? "Удалить разделение" : "Разделить здесь"}
                      >
        ...
        <div style={{ marginBottom: 16 }}>
          <p><strong>Предпросмотр частей:</strong></p>
          {(() => {
            const words = splitTextModal.textContent ? splitTextModal.textContent.split(/(\s+)/) : [];
            let parts: string[] = [];
            let current = "";
